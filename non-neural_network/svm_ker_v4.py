"""
ELEC 378 Final Project — Non-Neural-Network Model (v4)
Pipeline: SPM BoVW (500 words) + HSV Color + PCA + Chi-Squared Kernel SVM

Validation accuracy: 72.41%

Over v3, this version switches from an RBF kernel to a chi-squared
kernel. BoVW histograms are frequency-based features, and the
chi-squared distance is a natural similarity measure for histogram
data — this gave a large accuracy boost. PCA (250 components) is
still applied after standardization.

Run:  python pipeline_v4.py
"""
...
OUTPUT_DIR    = os.path.join(BASE_DIR, "v4_output")
...
VOCAB_SIZE     = 500
SAMPLE_PER_IMG = 50
PCA_COMPONENTS = 250        # still using PCA for dimensionality reduction
RANDOM_STATE   = 42
...
def main():
    t0 = time.time()
    print("=" * 65)
    print("PIPELINE v4: SPM BoVW (500) + Color + PCA + Chi² SVM")
    print("=" * 65)
    ...
    # 6. Standardize + PCA (still applied before chi² kernel)
    print(f"\n--- Phase 4: Standardize + PCA ({PCA_COMPONENTS} components) ---")
    ...
    print(f"  {total_dim} -> {PCA_COMPONENTS} dims (explains {explained*100:.2f}% variance)")

    # Shift to non-negative for chi² kernel (PCA output can be negative)
    min_val = min(X_train_pca.min(), X_val_pca.min())
    if min_val < 0:
        X_train_pca = X_train_pca - min_val
        X_val_pca   = X_val_pca - min_val
        print(f"  Shifted features by {-min_val:.4f} to ensure non-negativity for chi²")

    # 7. Chi-squared kernel SVM search
    gamma_values = [0.01, 0.05, 0.1, 0.5, 1.0, 5.0]
    C_values     = [1, 10, 50, 100]

    print(f"\n--- Phase 5: Chi-squared kernel SVM search ---")
    best_score = 0
    best_params = {}
    results = []
    ...
    for gamma in gamma_values:
        ...
        t_k = time.time()
        K_train = chi2_kernel(X_train_pca, X_train_pca, gamma=gamma)
    ...
    K_train_final = chi2_kernel(X_train_pca, X_train_pca, gamma=best_params['gamma'])
    ...
    # 9. Evaluate
    print(f"\n--- Phase 7: Evaluate ---")
    K_val = chi2_kernel(X_val_pca, X_train_pca, gamma=best_params['gamma'])
    ...
    pipeline = {
        ...
        'val_accuracy': val_acc, 'pca_shift': -min_val if min_val < 0 else 0,
    }
    with open(os.path.join(OUTPUT_DIR, "pipeline_v4.pkl"), "wb") as f:
    ...
    out_path = os.path.join(BASE_DIR, "submission_v4.csv")
    ...
    print(f"  PCA components:       {PCA_COMPONENTS} (explains {explained*100:.2f}%)")
    print(f"  Kernel:               Chi-squared (precomputed, on PCA features)")
