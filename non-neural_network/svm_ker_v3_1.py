"""
ELEC 378 Final Project — Non-Neural-Network Model (v3)
Pipeline: SPM BoVW (500 words) + HSV Color + PCA + RBF Kernel SVM

Validation accuracy: 66.34%

Over v2, this version reduces the visual vocabulary from 1000 to
500 words (less noisy clusters) and switches PCA from 95%-variance
to a fixed 250 components to keep more discriminative dimensions.
The hyperparameter grid is also refined with explicit gamma values
instead of 'scale'/'auto'.
...
OUTPUT_DIR    = os.path.join(BASE_DIR, "v3_output")
os.makedirs(OUTPUT_DIR, exist_ok=True)

# ── Parameters ────────────────────────────────────────────────
IMG_SIZE       = 256
SIFT_MAX_KP    = 500
VOCAB_SIZE     = 500        # CHANGED: reduced from 1000 to 500
SAMPLE_PER_IMG = 50
PCA_COMPONENTS = 250        # CHANGED: fixed 250 instead of 95% variance
RANDOM_STATE   = 42
...
def main():
    t0 = time.time()
    print("=" * 65)
    print("PIPELINE v3: SPM BoVW (500) + Color + PCA + RBF SVM")
    print("=" * 65)
    ...
    # 7. SVM hyperparameter search (refined grid with explicit gammas)
    ...
    with open(os.path.join(OUTPUT_DIR, "pipeline_v3.pkl"), "wb") as f:
    ...
    out_path = os.path.join(BASE_DIR, "submission_v3.csv")
